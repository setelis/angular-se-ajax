...
module.exports = function (grunt) {
	...
	function initConfig() {
		function getValidateTasks(allTasks) {
			if (process.env.SEAJAX_FAST === "true") {
				return [];
			}
			if (process.env.SEAJAX_W3C_LOCAL_URL && allTasks) {
				return [
					"jshint",
					"karma:unit",
					// "htmlangular:index",
		...
		grunt.initConfig({
			...
			htmlangular: {
				options: {
					...
					w3clocal: process.env.SEAJAX_W3C_LOCAL_URL?process.env.SEAJAX_W3C_LOCAL_URL:null,
					//i'm not sure why in local instalation these errors exists:
					relaxerror: process.env.SEAJAX_W3C_LOCAL_URL?[
	...
	function getBrowsersForTest() {
		if (process.env.SEAJAX_BROWSERS_FOR_TEST) {
			return process.env.SEAJAX_BROWSERS_FOR_TEST.split(",");
		} else {
